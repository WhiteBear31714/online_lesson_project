...
const configContainer = document.querySelector(".config-container");// กล่องการตั้งค่าหัวข้อ & จำนวนคำถา
const quizContainer = document.querySelector(".quiz-container");//กล่องแสดงคำถาม
const answerOptions = quizContainer.querySelector(".answer-options");//ตัวเลือก
const nextQuestionBtn = quizContainer.querySelector(".next-question-btn");//
const questionStatus = quizContainer.querySelector(".question-status");//แสดงสถานะ คำถามที่ x จากทั้งหมด y
const timerDisplay = quizContainer.querySelector(".timer-duration");//จับเวลา
const resultContainer = document.querySelector(".result-container");//ผลคะแนน

// Quiz state variables
const QUIZ_TIME_LIMIT = 15;
let currentTime = QUIZ_TIME_LIMIT;
let timer = null;
let quizCategory = "ก่อนเรียน";//หัวข้อ 
let numberOfQuestions = 20;//จำนวนคำถามทั้งหมด
let currentQuestion = null;
const questionsIndexHistory = [];
let correctAnswersCount = 0;//นับจำนวนคำถามที่ถูก
let disableSelection = false;//ป้องกันกดหลายครั้ง
